...
function readOptions(root: HTMLElement): CarouselOptions {
  ...
  const mode: CarouselMode =
    requestedMode === "autoplay" || requestedMode === "continuous"
      ? requestedMode
      : "manual";
  ...
  return {
    ...
    loop: readBoolean(root.dataset.carouselLoop, false),

    draggable: readBoolean(root.dataset.carouselDraggable, true),
    ...
    pauseOnHover: readBoolean(root.dataset.carouselPauseOnHover, true),

    pauseOnFocus: readBoolean(root.dataset.carouselPauseOnFocus, true),
  };
}

function getSlides(track: HTMLElement): HTMLElement[] {
  return Array.from(track.querySelectorAll<HTMLElement>(SLIDE_SELECTOR));
}
...
function getSlideStep(track: HTMLElement, viewport: HTMLElement): number {
  ...
  return firstSlide.getBoundingClientRect().width + getTrackGap(track);
}
...
function getVisibleSlideCount(
  ...
  const slideStep = getSlideStep(track, viewport);
  ...
  return Math.max(1, Math.floor((viewport.clientWidth + gap) / slideStep));
}

function initializeCarousel(root: HTMLElement): void {
  if (root.dataset[INITIALIZED_ATTRIBUTE] === "true") {
    return;
  }
  ...
  const clampScrollPosition = (position: number): number => {
    return Math.min(getMaximumScroll(), Math.max(0, position));
  };

  const updateControlsForPosition = (position: number): void => {
    ...
    if (options.loop && maximumScroll > SCROLL_TOLERANCE) {
    ...
    const canScroll = maximumScroll > SCROLL_TOLERANCE;

    previousButton.disabled = !canScroll || clampedPosition <= SCROLL_TOLERANCE;

    nextButton.disabled =
      !canScroll || clampedPosition >= maximumScroll - SCROLL_TOLERANCE;
  };
  ...
  const finishPendingScroll = (): void => {
    ...
    updateControlsForPosition(viewport.scrollLeft);
  };

  const updateControls = (): void => {
    updateControlsForPosition(pendingScrollTarget ?? viewport.scrollLeft);
  };

  const getNavigationDistance = (): number => {
    const slideStep = getSlideStep(track, viewport);

    const visibleSlideCount = getVisibleSlideCount(track, viewport);

    return slideStep * visibleSlideCount;
  };

  const scrollToPosition = (position: number): void => {
    const targetPosition = clampScrollPosition(position);
  ...
  const scrollByPage = (direction: -1 | 1): void => {
    const currentPosition = pendingScrollTarget ?? viewport.scrollLeft;

    const targetPosition =
      currentPosition + getNavigationDistance() * direction;
  ...
  const startAutoplay = (): void => {
    ...
    autoplayTimer = window.setInterval(() => {
      ...
      if (currentPosition >= maximumScroll - SCROLL_TOLERANCE) {
        if (options.loop) {
          scrollToPosition(0);
        }
  ...
  viewport.addEventListener("scrollend", finishPendingScroll);
  ...
  if (options.draggable) {
    viewport.classList.add("carousel__viewport--draggable");
...
export function initCarousels(): void {
  const carousels = document.querySelectorAll<HTMLElement>(CAROUSEL_SELECTOR);
